export const Hours = () => {
  return (
    <section>
      <header>
        <h1 className="text-7xl">Hours</h1>
        <p className="text-neutral-gray">Lorem, ipsum dolor sit amet consectetur adipisicing elit. Impedit.</p>
      </header>
      <article>
        <ul>
          <li className="group flex justify-between items-center py-4 px-4">
            <span className="group-hover:font-bold group-hover:text-primary">Monday</span>
            <span className="group-hover:font-bold group-hover:text-primary">09:00 am to 22:30 pm</span>
          </li>
          <li className="group flex justify-between items-center py-4 px-4">
            <span className="group-hover:font-bold group-hover:text-primary">Monday</span>
            <span className="group-hover:font-bold group-hover:text-primary">09:00 am to 22:30 pm</span>
          </li>
          <li className="group flex justify-between items-center py-4 px-4">
            <span className="group-hover:font-bold group-hover:text-primary">Monday</span>
            <span className="group-hover:font-bold group-hover:text-primary">09:00 am to 22:30 pm</span>
          </li>
          <li className="group flex justify-between items-center py-4 px-4">
            <span className="group-hover:font-bold group-hover:text-primary">Monday</span>
            <span className="group-hover:font-bold group-hover:text-primary">09:00 am to 22:30 pm</span>
          </li>
          <li className="group flex justify-between items-center py-4 px-4">
            <span className="group-hover:font-bold group-hover:text-primary">Monday</span>
            <span className="group-hover:font-bold group-hover:text-primary">09:00 am to 22:30 pm</span>
          </li>
        </ul>
      </article>
    </section>
  )
}
